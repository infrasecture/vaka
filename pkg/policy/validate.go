...
const (
	validationHost validationMode = iota
	validationInjected
)
...
func Validate(p *ServicePolicy, networkModes map[string]string) []error {
	return ValidateHost(p, networkModes)
}

// ValidateHost validates a user-authored host vaka.yaml document.
// Generated fields (generatedBy, requiredRuntimeVersion, and
// services.<name>.user) are rejected.
func ValidateHost(p *ServicePolicy, networkModes map[string]string) []error {
	return validateWithMode(p, networkModes, validationHost)
}

// ValidateInjected validates an injected per-service policy document consumed by
// vaka-init inside the container. Generated fields are allowed;
// requiredRuntimeVersion is required.
func ValidateInjected(p *ServicePolicy) []error {
	return validateWithMode(p, nil, validationInjected)
}

func validateWithMode(p *ServicePolicy, networkModes map[string]string, mode validationMode) []error {
	var errs []error
	...
	if mode == validationHost && p.GeneratedBy != "" {
		add("generatedBy: must not be set in vaka.yaml (it is injected by the vaka CLI)")
	}
	if mode == validationHost && p.RequiredRuntimeVersion != "" {
		add("requiredRuntimeVersion: must not be set in vaka.yaml (it is injected by the vaka CLI)")
	}
	if mode == validationInjected && p.RequiredRuntimeVersion == "" {
		add("requiredRuntimeVersion: missing — policy was generated without a runtime compatibility requirement")
	}
