...
// Generated fields (vakaVersion, services.<name>.user) are rejected.
...
// vaka-init inside the container. Generated fields are allowed; vakaVersion is
// required.
...
func validateWithMode(p *ServicePolicy, networkModes map[string]string, mode validationMode) []error {
	...
	if mode == validationHost && p.VakaVersion != "" {
		add("vakaVersion: must not be set in vaka.yaml (it is injected by the vaka CLI)")
	}
	if mode == validationInjected && p.VakaVersion == "" {
		add("vakaVersion: missing — policy was generated by an incompatible or unknown CLI version")
	}
