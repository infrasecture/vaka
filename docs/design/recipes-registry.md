...
**Published distribution format.** A release registry is two kinds of static
HTTPS artifacts, generated by the repo's CI:
...
For pre-release testing, Vaka also supports an explicitly configured **Git
preview registry**. It resolves one mutable ref to an immutable commit on
`registry add-git` or `registry refresh`, packages only tracked top-level recipe
directories into a local content-addressed cache, and generates the same index
model locally. Normal catalog and `get` operations consume that generated cache
and never contact Git. This is a development channel, not an alternate release
transport or publication authority.
...
vaka caches each published registry's `index.yaml` (with
ETag/If-Modified-Since) under the user cache dir; `vaka registry refresh`
forces re-fetch. A Git preview stores a locally generated index, its resolved
commit, and digest-addressed artifacts in the same per-registry cache area.
Search and `info` work entirely from cached indexes — no Git operation and no
network round-trip per query. The atomically renamed cache envelope is bound to
the complete source identity: index URL, or Git URL plus ref. Re-pointing any
part is a cache miss, and readers never pair an index with another source's
provenance. (Signing will extend published source identity with the pinned
`publicKey`; §9 Phase 3.)
...
registries:
  ...
  - name: preview                 # added via `vaka registry add-git`
    git:
...
- Unqualified `codex` → resolved **only if the name is unique across all
  configured published registries**; otherwise vaka errors and lists the
  qualified candidates. This is the anti-typosquatting rule: adding a registry can never
  ...
  only warn.) Git previews are excluded from this candidate set and therefore
  cannot hijack or make a published name ambiguous.
- Git preview recipes always require a qualified reference such as
  `preview/codex`. An update-in-place remains qualified implicitly because its
  lock supplies the registry name.
- `official` is built in; removing it is allowed but explicit.

### Git preview lifecycle

`vaka registry add-git preview <git-url> --ref <branch>` validates the source,
resolves it to a full commit ID, and builds the initial cache before saving the
configuration. A shorthand ref means `refs/heads/<ref>`; callers can provide a
...
environment-injected config variables are removed from subprocesses so they
cannot redirect the temporary object store. Normal Git credential helpers,
SSH agents, and explicitly configured SSH commands remain available. Supported
sources are HTTPS, SSH, scp-style SSH, and absolute `file://`; unsafe transports
...
version: 0.3.0
digest: sha256:9f2a...
sourceRevision: 4a92...                  # Git previews only: full commit ID
...
configured registry's published or locally generated cached index; it never
fetches an authoring repository and never scans the local filesystem for
instantiated recipes. Only explicit Git add/refresh operations access a Git
repository and rebuild a preview catalog.
...
- Git previews are explicitly opt-in mutable development sources. Every
  refresh resolves the ref once, records the full commit, packages only commit
  objects, and binds the result to a sha256 artifact digest. They do not claim
  the release trust or immutability of a published registry.
...
- `pkg/registry`: registries config load/save, published index fetch+cache
  (ETag), explicit Git preview resolution/packaging, index model, name
  resolution (uniqueness rule), semver selection.
- `pkg/recipe`: manifest + lock models, a shared safe recipe-dir I/O
  ...
  `loadAndValidate`).
- `cmd/vaka`: `get.go`, `search.go`, `recipes.go`, `registry_cmd.go` — normal
...
hand-roll SemVer handling. Published registries add no executable dependency;
Git preview add/refresh uses the user's mature `git` executable so existing
credential and SSH configuration work without embedding another Git stack.
There is no go-git dependency and no YAML library beyond the existing
`gopkg.in/yaml.v3`.
...
2. **Phase 2 — management & polish**: `vaka registry add|remove|refresh`
   ...
   publisher's call. Git-backed, explicitly refreshed preview registries allow
   branch candidates to be tested before publication without weakening the
   immutable release registry. (See "Deferred hardening" below for pinning the official
   recipe's own images.)
